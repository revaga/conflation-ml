...
from scripts.phonenumber_validator import validate_phone_number, try_with_region
...
def generate_golden_label(row):
    ...
    # STEP 2 — Evaluate each core attribute
    ...
    def _phone_str(val):
        """Coerce to string for phone processing."""
    ...
    if base_list or alt_list:
        ...
        if addr_winner is None:
            ...
            if base_complete > alt_complete:
                ...
                addr_winner = "alt"
    if addr_winner is None:
        addr_winner = _prompt_superiority("Address", _addr_show(base_addr_raw), _addr_show(alt_addr_raw))
...
if __name__ == "__main__":
    ...
    if OUTPUT_PATH.exists():
        ...
        logger.info(f"Resuming: {n_done} records already labeled in {OUTPUT_PATH}")
    else:
        results_df = pd.DataFrame()

    for i in range(n_done, NUM_RECORDS):
        row = subset.iloc[i]
        print(f"\n{'='*60} Record {i + 1} / {NUM_RECORDS} {'='*60}")
        ...
        if i < NUM_RECORDS - 1:
            choice = input("\nPress Enter for next, or 'quit' to save: ").strip().lower()
            if choice in ("quit", "q", "pause", "p"):
                break

    _save_golden(results_df, OUTPUT_PATH)
    logger.info(f"Complete. Saved {len(results_df)} records to {OUTPUT_PATH}")